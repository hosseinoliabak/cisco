...
0. Is NEXT_HOP (Well-known Mandatory) in the routing table?
  * No, reject the process for that route.
  * Yes, continue the process.
1. Highest NX-OS weight
...
2. Highest Local Preference (Well-known discretionary)
...
6. Lowest Multi-Exit Discriminator (MED) (Optional Non-transitive)
...
8. The lowest IGP metric for reaching the NEXT_HOP:
9. Oldest (longest-known) eBGP route
10. Lowest neighbor BGP RID
11. Shortest CLUSTER_LIST in route reflector topologies
12. Lowest iBGP neighbor IP address
...
N9K01(config-router-stmp)# password SMENODEPASS
