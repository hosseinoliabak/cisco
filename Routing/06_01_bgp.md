...
* Network Layer Reachability Information (NLRI) which is unique to BGP version 4
...
We can inject routes using `network` command or using *redistribution*.
...
And this loop keeps going on until the TTL expires. We have 2 solutions:
1. Redistribute the eBGP to our IGP which is not efficient at all.
2. Bringing Core1 and Core2 in iBGP play then run a full mesh iBGP between them.